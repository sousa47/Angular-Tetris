...
export class TPiece extends TetrisPiece {
  ...
  private _pieceColor: string = 'purple';

  // Orientation is based of the middle piece.
  ...
  override drawPiece(
    ...
    // adding the above piece in the middle
    this._canvas!.fillRect(
      this._xCoordinates + widthLength / 3,
      this._yCoordinates,
      widthLength / 3,
      heightLength
    );
  ...
  private drawBorderAndInnerBorder(): void {
    ...
    this._canvas!.strokeRect(
      this._xCoordinates + this._widthLength! / 3,
      this._yCoordinates,
      this._widthLength! / 3,
      this._heightLength!
    );
